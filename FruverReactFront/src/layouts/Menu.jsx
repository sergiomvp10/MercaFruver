                          ...
                          <>
                            <div className="flex-1 overflow-y-auto">
                              {(() => {
                                ...
                                const left = CASH_DENOMINATIONS.slice(0, half);
                                const right = CASH_DENOMINATIONS.slice(half);
                                const Block = (denom) => (
                                  <div key={denom} className="grid grid-cols-3 items-center gap-2 border-b py-2">
                                    <div className="font-medium">{formatMoney(denom)}</div>
                                    <input
                                      type="number"
                                      min="0"
                                      ...
                                      placeholder="0"
                                    />
                                    <div className="text-right font-medium">{formatMoney((parseInt(cashCounts[denom]) || 0) * denom)}</div>
                                  </div>
                                );
                                return (
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>{left.map(Block)}</div>
                                    <div>{right.map(Block)}</div>
                                  </div>
                                );
                              })()}
                            </div>
