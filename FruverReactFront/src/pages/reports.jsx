...
const Reports = () => {
  ...
  const fetchMonthlyReport = useCallback(async () => {
    ...
    const fetchCashCloseReport = useCallback(async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${API}/cashclose/daily-report`, {
          params: { date: selectedDate }
        });
        setCashCloseData(response.data);
      } catch (error) {
        console.error("Error fetching cash close report:", error);
      }
      setLoading(false);
    }, [selectedDate]);
    ...
    useEffect(() => {
      if (isAuthenticated) {
        if (activeTab === "daily") {
          ...
          fetchRangeReport();
        } else if (activeTab === "cashclose") {
          fetchCashCloseReport();
        }
      }
    }, [isAuthenticated, activeTab, fetchDailyReport, fetchMonthlyReport, fetchRangeReport, fetchCashCloseReport]);
              ...
              {loading ? (
                ...
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 p-4 rounded-xl text-white shadow-lg">
                      <h3 className="text-sm opacity-80">Total Ventas Sistema</h3>
                      <p className="text-2xl font-bold mt-1">{moneyFormat(cashCloseData.ventas?.totalVentas || 0)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-green-500 to-green-600 p-4 rounded-xl text-white shadow-lg">
                      <h3 className="text-sm opacity-80">Ventas en Efectivo</h3>
                      <p className="text-2xl font-bold mt-1">{moneyFormat(cashCloseData.ventas?.totalEfectivo || 0)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-4 rounded-xl text-white shadow-lg">
                      <h3 className="text-sm opacity-80">Transferencias (BRE-B)</h3>
                      <p className="text-2xl font-bold mt-1">{moneyFormat(cashCloseData.ventas?.totalTransferencia || 0)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-4 rounded-xl text-white shadow-lg">
                      <h3 className="text-sm opacity-80">Cantidad de Ventas</h3>
                      <p className="text-2xl font-bold mt-1">{cashCloseData.ventas?.cantidadVentas || 0}</p>
                    </div>
                  </div>

                  {cashCloseData.cierresCaja && cashCloseData.cierresCaja.length > 0 ? (
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                      <h3 className="bg-gray-100 px-6 py-3 font-semibold text-gray-700">Cierres de Caja del Dia</h3>
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Ventas Sistema</th>
                              <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Efectivo Contado</th>
                              <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Diferencia</th>
                              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Estado</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-200">
                            {cashCloseData.cierresCaja.map((cierre, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{cierre.dateFormatted}</td>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{cierre.userName || 'N/A'}</td>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-right font-medium">{moneyFormat(cierre.systemTotal || 0)}</td>
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-right font-medium">{moneyFormat(cierre.cashTotal || 0)}</td>
                                <td className={`px-4 py-4 whitespace-nowrap text-sm text-right font-bold ${cierre.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {cierre.difference >= 0 ? '+' : ''}{moneyFormat(cierre.difference || 0)}
                                </td>
                                <td className="px-4 py-4 whitespace-nowrap text-center">
                                  {cierre.difference === 0 ? (
                                    <span className="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Cuadrado</span>
                                  ) : cierre.difference > 0 ? (
                                    <span className="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">Sobrante</span>
                                  ) : (
                                    <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Faltante</span>
                                  )}
                                </td>
                              </tr>
                            ))}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                      <p className="text-yellow-700">No hay cierres de caja registrados para esta fecha</p>
                      <p className="text-yellow-600 text-sm mt-2">Los empleados pueden registrar cierres desde el boton "Cerrar Caja" en el menu</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-10 text-gray-500">
                  Seleccione una fecha para ver el reporte de cierre de caja
                </div>
              )}
            </div>
          )}
